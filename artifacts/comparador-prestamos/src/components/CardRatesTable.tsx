...
import { visaCards, mastercardCards, amexCards, cabalCards, naranjaCards, CardBrand, BRAND_INFO } from '@/data/cards';
import { formatPercentage } from '@/lib/calculations';
import { Badge } from '@/components/ui/badge';
import { Trophy, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';

const ALL_BRANDS: CardBrand[] = ['visa', 'mastercard', 'amex', 'cabal', 'naranja'];
...
export function CardRatesTable() {
  ...
  const totalBanks = ALL_BRANDS.reduce((acc, b) => acc + DATA[b].length, 0);

  return (
    <div className="bg-white rounded-2xl shadow-lg border border-slate-200/80 overflow-hidden">
      {/* Header — clickable toggle */}
      <div
        className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white cursor-pointer flex items-center justify-between"
        onClick={() => setOpen(o => !o)}
      >
        <div>
          ...
          {!open && (
            <p className="text-xs text-slate-400 mt-1">
              {totalBanks} bancos · Visa, Mastercard, Amex, Cabal, Naranja — click para ver
            </p>
          )}
          {open && (
      ...
      {open && (
        <>
          ...
          {/* Footer note */}
          <div className="px-6 py-3 bg-amber-50 border-t border-amber-100 flex flex-wrap items-center justify-between gap-2">
            ...
            <p className="text-xs text-slate-400 shrink-0">Tasas mayo 2026 · Fuente: BCRA y sitios oficiales</p>
          </div>
        </>
      )}
    </div>
